...
function tutor()
{
	...
	switch(step)
	{
		...
		document.getElementById('tutora').href="#";tutora.onclick=function(){chujizhaomo();tutor();};
		break;
		case 5:
		document.getElementById('maskscreen').style.display="block";
		document.getElementById('tutorscreen').style.display="none";
		...
		tutora.onclick=function(){zhongjizhaomo();tutor();}
		//document.getElementById('maskscreen').style.display="none";
		break;
		default:
		document.getElementById('maskscreen').style.display="none";
...
function selecthero(x)
{
	...
	document.getElementById("mywj0"+x).style.borderColor="#F90"; document.getElementById("mywj0"+x).style.borderStyle="solid";
	for(i=1;i<=heronum;i++){
		if(i==x)
...
function showselecthero()
{
	...
	document.getElementById("startpk").onclick = function(){pk();}
}
...
{
	if(x==0){alert("主公，我输了，再给我一次机会，让我将战斗进行到底吧！");}
	if(x==1){
...
function fight()
{
	...
	if(selectedhero==-1){alert("请主公选择武将");exit(1);}
...
function physicalattack()
{
	...
	setTimeout(function(){document.getElementById('pkpic2b').style.display="none";},500);
	if(pklife2<=0){
		fightuser[stageid]=username;
		fightstage[stageid]=encode(selectedhero,1);
		document.getElementById('resultpic').src="pic/pk/win.png";
		document.getElementById('resultdiv').style.display="block";
		setTimeout(function(){pkover()},3000);
	}else setTimeout(function(){AIattack()},1000);
}
...
function AIattack()
{
	...
	setTimeout(function(){document.getElementById('pkpic1b').style.display="none";},500);
...
function guyong(x)
{
	...
	zhaomu[x]-=10000;
	if(wjinfo[0][x]==0){cqnum++;document.getElementById("chuanqi").innerHTML=cqnum;}
	if(wjinfo[0][x]==1){ylnum++;document.getElementById("yiliu").innerHTML=ylnum;}
...
switch(checkclass){
case 0:document.getElementById("heroclass").src="pic/gamelogo/wjclass0.png"; break;
case 1:document.getElementById("heroclass").src="pic/gamelogo/wjclass1.png"; break;
case 2:document.getElementById("heroclass").src="pic/gamelogo/wjclass2.png"; break;
case 3:document.getElementById("heroclass").src="pic/gamelogo/wjclass3.png"; break;
case 4:document.getElementById("heroclass").src="pic/gamelogo/wjclass4.png"; break;
}
...
function checkhero(x)
{
	switch(x)
	{
		case id(hero[1]):return 1;break;
		case id(hero[2]):return 1;break;
		case id(hero[3]):return 1;break;
		case id(hero[4]):return 1;break;
		case id(hero[5]):return 1;break;
		case id(hero[6]):return 1;break;
		case id(hero[7]):return 1;break;
		case id(hero[8]):return 1;break;
		case id(hero[9]):return 1;break;
		case id(hero[10]):return 1;break;
		case id(hero[11]):return 1;break;
		case id(hero[12]):return 1;break;
		default:return 0;
	}
}
...
function xj(x,y)               //x--武将等级，y--所在窗口
{
	...
	if(x<12)
	{
	do{num=Math.random(1,37);}while(checkhero(num)==1);
	...
	if(x>=12&&x<52)
	{
	do{num=Math.random(38,137);}while(checkhero(num)==1);
	...
	if(x>=53&&x<800)
	{
	do{num=Math.random(131,278);}while(checkhero(num)==1);
	...
	if(x>=801&&x<3000)
	{
	do{num=Math.random(279,456);}while(checkhero(num)==1);
	...
	if(x>=3000)
	{
	do{num=Math.random(457,670);}while(checkhero(num)==1);
...
function chujizhaomo()
{
    ...
    xj(possiblea,1);
    xj(possibleb,2);
	wjinfo[1][3]=null;zhaomu[3]=null;
	wjinfo[1][4]=null;zhaomu[4]=null;
	wjinfo[1][5]=null;zhaomu[5]=null;
	wjinfo[1][6]=null;zhaomu[6]=null;
    //wujiang();
//updatecookie();
update();
}
